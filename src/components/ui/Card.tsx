...
export default function Card({ children, className = "", padded = true, onClick, style }: {
  children: React.ReactNode; className?: string; padded?: boolean; onClick?: () => void; style?: React.CSSProperties;
}) {
  return (
    <div
      onClick={onClick}
      className={`bg-white rounded-2xl ${padded ? "p-4" : ""} ${className}`}
      style={{ boxShadow: "0 1px 2px rgba(20,24,31,0.04), 0 8px 24px rgba(20,24,31,0.04)", ...style }}
    >
      {children}
    </div>
  );
